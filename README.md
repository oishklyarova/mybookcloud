📚 MyBookCloud – Event-Driven Book Management System
   ...
   MyBookCloud is a distributed event-driven application built with .NET 7, Angular, MassTransit, and SignalR.
...
🚀 Key Features
   ...
   Uses RabbitMQ for asynchronous data enrichment
...
💡 Notes
   ...
   The Worker service implements Retries with Exponential Backoff and the Circuit Breaker pattern to handle unstable external dependencies (like the Google Books API). This prevents system degradation if the third-party service is temporarily unavailable.